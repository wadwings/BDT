...
def getinfor(city_s,infor,soup,date):
    while updated(soup,date+2)==1:
        data_time=soup.select('#content > table > tr:nth-child({}) > td:nth-child(1) > a'.format(str(date+2)))
        data_weather=soup.select('#content > table > tr:nth-child({}) > td:nth-child(2)'.format(str(date+2)))
        data_temperature=soup.select('#content > table > tr:nth-child({}) > td:nth-child(3)'.format(str(date+2)))
        data_wind=soup.select('#content > table > tr:nth-child({}) > td:nth-child(4)'.format(str(date+2)))
        time=data_time[0].string.replace(' ','').replace('\n','').replace('\r','')
        weather=data_weather[0].string.replace(' ','').replace('\n','').replace('\r','')
        temperature=data_temperature[0].string.replace(' ','').replace('\n','').replace('\r','')
        wind=data_wind[0].string.replace(' ','').replace('\n','').replace('\r','')
        infor.append({'city':city_s,'time':time,'weather':weather,'temperature':temperature,'wind':wind})
        date+=1
    return infor
