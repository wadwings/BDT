...
import time
import random
target_year_list = ["2014","2015","2016","2017","2018","2019","2020"]
...
def requestDemo(url,headers):
    ...
    for i in range(trytimes):
        ...
        # logdebug(f'requests failed {i}time')
            f = open('fault.1','a',encoding='utf-8')
...
for i in city.keys():
    ...
    for year in target_year_list:
        for month in target_month_list:
            date = year + month
            url = 'http://www.tianqihoubao.com/aqi/' + i + '-{}.html'.format(date)
            ua = UserAgent()
            ...
            if strhtml.status_code == 200:#网页正常访问
                ...
                commid2=soup.find_all('div',class_='api_month_list')
                for info in commid2:
                  ...
                  for index, tr in enumerate(tr_list):     # enumerate可以返回元素的位置及内容
                        ...
                        infor[count]['level'] = level
                        infor[count]['aqi'] = aqi
                        infor[count]['rank'] = rank
                        infor[count]['PM2.5'] = PM2
                        infor[count]['PM10'] = PM10
                        infor[count]['SO2'] = SO2
                        infor[count]['NO2'] = NO2
                        infor[count]['CO'] = CO
                        infor[count]['O3'] = O3
